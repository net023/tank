...
public class BaiDuLBSTool {
	...
	// 更新poi
	public static String updatePoi(String geotable_id, String ak, String title,
			String address, String tags, Double latitude, Double longitude,
			Integer id) {
		HttpResponse send = HttpRequest.post(UPDATEPOI).form("title", title)
				.form("address", address).form("tags", tags)
				.form("latitude", latitude).form("longitude", longitude)
				.form("coord_type", 3).form("geotable_id", geotable_id)
				.form("ak", ak).form("id", id).send();
		return send.body();
	}

	// 删除poi
	public static String deletePoi(String geotable_id, String ak, Integer id) {
		HttpResponse send = HttpRequest.post(DELETEPOI)
				.form("geotable_id", geotable_id).form("ak", ak).form("id", id)
				.send();
		return send.body();
	}
	...
	public static void main(String[] args) {
		// 创建数据库
//		 System.out.println(createGeoTable("快卡_测试", 1, 1, AK));
		// 107962
		 //112130
		...
		// 更新poi
		// System.out.println(updatePoi("107962", AK, "xxx", "观音桥xxx", null,
		// 106.32, 60.55, 945969865));
